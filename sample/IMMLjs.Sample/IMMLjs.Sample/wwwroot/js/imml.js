...
function _fetchImmlFromUri(uri, completed) {
    ...
    oReq.onload = function (e) {
        //var xDoc = oReq.responseText;

        var parser = new DOMParser();
        var xDoc = parser.parseFromString(oReq.responseText, "application/xml");
        //console.log("reponse text");
        //console.log(oReq.responseText);
        //console.log("reponse XML");
        //console.dir(oReq.responseXML);
...
imml.fetchIncludes = function (immlNode, completed) {
    ...
    var pendingFetch = includes.length;
        ...
        if (src.length > 0) {
            _fetchImmlFromUri(src, function (xDoc) {
                ...
                if (xDoc != null) {
                    ...
                    //xdoc has parent IMML element
                    for (var j = 0; j < xDoc.documentElement.children.length; j++) {
                        immlNode.appendChild(xDoc.documentElement.children[j]);
                    }
                }
...
imml.load = function (parentElement) {
    ...
    //use a hemispheric light to simulate globalillumination
    var globalIllumination = _stringToColour3(immlNode.getAttribute('GlobalIllumination'));

    var hemiLight = new BABYLON.HemisphericLight("GlobalIllumination", new BABYLON.Vector3(0, 1, 0), scene);
    hemiLight.groundColor = globalIllumination;
...
function _loadPhysics(item, mesh, scene) {
    ...
    if (physicsNodes.length > 0) {
        ...
        var weight = movable ? _floatOrDefault(physicsNode.getAttribute('Weight'), 0) : 0;
        ...
        if (interactionNodes.length > 0) {
            ...
            mesh.physicsImpostor = new BABYLON.PhysicsImpostor(mesh, imposter, { mass: weight }, scene);
        }
...
imml.loadModels = function (immlNode, scene) {
    //<model source="models\somemodel.gltf" >...</model>
